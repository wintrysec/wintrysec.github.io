import{_ as a}from"./_plugin-vue_export-helper.cdc0426e.js";import{o as l,c as d,a as n,b as s,d as r,e,r as t}from"./app.f283cffb.js";const c="/assets/linux_deamon.e9f095c4.png",v={},o=e('<h2 id="守护进程概述" tabindex="-1"><a class="header-anchor" href="#守护进程概述" aria-hidden="true">#</a> 守护进程概述</h2><p>守护进程（daemon）就是一直在后台运行的进程,一般是为了保护我们的程序/服务的正常运行，当程序被关闭、异常退出等时再次启动程序/恢复服务。</p><p>需要让程序在后台不间断的运行可以这样做</p><div class="language-bash line-numbers-mode" data-ext="sh"><pre class="language-bash"><code><span class="token function">nohup</span> <span class="token variable"><span class="token variable">`</span>./api<span class="token variable">`</span></span> <span class="token operator">&amp;</span>\n</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><p>反引号的作用是执行完此条命令终端不卡住。</p><p>也可以写成Daemon程序，例如 Apache 、MongoDB等，守护进程的名称通常以d结尾，比如sshd、httpd、mongod等。</p><p>守护进程脱离终端而存在。当你执行命令<code>ps -o pid,ppid,cmd -p 7390</code> 时，守护进程的 PPID （父进程 ID）都是 1，TTY （终端）则是<code>???</code>。</p><h2 id="golang实现守护进程" tabindex="-1"><a class="header-anchor" href="#golang实现守护进程" aria-hidden="true">#</a> Golang实现守护进程</h2><p>守护进程创建流程</p><p><img src="'+c+'" alt="linux_deamon" loading="lazy"></p><p>1、frok创建子进程，并exit使父进程退出</p><p>既然是服务进程，意味着是可以独立运行的，不会因为父进程退出而销毁，在 Linux 系统中有一个进程号为 1 的 init 进程，这个进程一直存在与系统中，当我们的子进程从父进程脱离后，子进程变为孤儿进程，随之系统的 init 进程会接管对这个进程的控制。</p><p>2、重启session会话</p><p>使用setsid重新开启一个session会话，使当前进程脱离原会话、原进程组、原终端的控制</p><p>3、改变工作目录</p><p>进程活动时，其工作目录所在的文件系统不能卸载，一般需要将守护进程工作目录改变到根目录</p><p>4、umask 重设文件权限掩码</p><p>从父进程继承文件创建的掩码，可能修改守护进程所创建的文件的权限，需要清除重设</p><p>5、关闭文件描述符</p><p>从父进程继承了打开的文件描述符，若不关闭将会造成资源浪费，及其它未知错误</p><p>6、重定向标准输入、输出、错误描述符至NULL</p><p>守护进程在后台运行，不会与用户发生直接的交互，我们希望限制终端的输入输出，因此将文件描述符 0, 1, 2 定位到 <code>/dev/null</code></p>',22),p={href:"https://github.com/sevlyar/go-daemon",target:"_blank",rel:"noopener noreferrer"},u=e(`<p>或者自己编码实现：</p><div class="language-golang line-numbers-mode" data-ext="golang"><pre class="language-golang"><code>func Daemon() {
	//父进程为init，已经是一个守护进程
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h2 id="systemd" tabindex="-1"><a class="header-anchor" href="#systemd" aria-hidden="true">#</a> systemd</h2><p>Linux系统有自己的守护进程管理工具 Systemd 。</p><p>它是操作系统的一部分，直接与内核交互，性能出色，功能极其强大。</p><p>我们完全可以将程序交给 Systemd ，让系统统一管理，成为真正意义上的系统服务。</p><div class="custom-container tip"><p class="custom-container-title">提示</p><p>例如以下是一个将FRP设置为服务的操作</p></div><div class="language-bash line-numbers-mode" data-ext="sh"><pre class="language-bash"><code><span class="token shebang important">#!/bin/bash</span>
...
Restart=on-failure
RestartSec=5s
WorkingDirectory=/usr/bin  # 工作目录（可选）
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h3 id="服务管理" tabindex="-1"><a class="header-anchor" href="#服务管理" aria-hidden="true">#</a> 服务管理</h3><div class="language-bash line-numbers-mode" data-ext="sh"><pre class="language-bash"><code><span class="token comment"># 启动服务</span>
systemctl start <span class="token operator">&lt;</span>服务名<span class="token operator">&gt;</span>  <span class="token comment"># 如 systemctl start sshd</span>
...
systemctl reload <span class="token operator">&lt;</span>服务名<span class="token operator">&gt;</span>

<span class="token comment"># 查看服务状态（active/running 为正常运行）</span>
...
<span class="token comment"># 查看服务是否开机自启</span>
systemctl is-enabled <span class="token operator">&lt;</span>服务名<span class="token operator">&gt;</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div>`,10);function m(b,f){const i=t("ExternalLinkIcon");return l(),d("div",null,[o,n("p",null,[s("有一个第三方库可以快速实现："),n("a",p,[s("https://github.com/sevlyar/go-daemon"),r(i)])]),u])}const k=a(v,[["render",m],["__file","Linux守护进程.html.vue"]]);export{k as default};
